...
public class PawnMovementTests
{
    ...
    public void TwoPawns_WalkingTowardEachOther_DoNotGetStuck()
    {
        ...
        for (int tick = 0; tick < 500; tick++)
        {
            ...
            if (leftPos.HasValue) leftPositions.Add(leftPos.Value);
            if (rightPos.HasValue) rightPositions.Add(rightPos.Value);
            ...
            if (tick % 50 == 0)
            {
                ...
                {
                    _output.WriteLine($"  Left action: {leftAction.CurrentAction?.DisplayName ?? "none"}, Queue: {leftAction.ActionQueue.Count}");
                }
                if (sim.Entities.Actions.TryGetValue(rightPawn.Value, out var rightAction))
                {
                    _output.WriteLine($"  Right action: {rightAction.CurrentAction?.DisplayName ?? "none"}, Queue: {rightAction.ActionQueue.Count}");
                }
            }
        }
        ...
        var lastLeftPositions = leftPositions.GetRange(Math.Max(0, leftPositions.Count - 100), Math.Min(100, leftPositions.Count));
        var lastRightPositions = rightPositions.GetRange(Math.Max(0, rightPositions.Count - 100), Math.Min(100, rightPositions.Count));
        ...
        _output.WriteLine($"Last 100 ticks - Left unique positions: {uniqueLeftPositions.Count}, Right unique positions: {uniqueRightPositions.Count}");
        ...
        Assert.True(uniqueLeftPositions.Count > 2 || uniqueRightPositions.Count > 2,
    ...
    public void TwoPawns_TryingToSwapPositions_DoNotDeadlock()
    {
        ...
        for (int tick = 0; tick < 300; tick++)
        {
            ...
            if (tick % 50 == 0)
            {
                ...
                _output.WriteLine($"Tick {tick}: Pawn1 at {pos1} (hunger={hunger1:F1}), Pawn2 at {pos2} (hunger={hunger2:F1})");
            }
        }

        _output.WriteLine($"Final: Pawn1 used fridge: {pawn1UsedFridge}, Pawn2 used fridge: {pawn2UsedFridge}");

        // Assert: At least one pawn should have been able to use a fridge
        Assert.True(pawn1UsedFridge || pawn2UsedFridge,
    ...
    public void TwoPawns_InWiderArea_CanNavigateAroundEachOther()
    {
        ...
        for (int tick = 0; tick < 400; tick++)
        {
            ...
            if (hunger2 > 40) fedCount++;

            if (tick % 100 == 0)
            {
                var pos1 = sim.GetPosition(topPawn.Value);
                var pos2 = sim.GetPosition(bottomPawn.Value);
                _output.WriteLine($"Tick {tick}: Top at {pos1} (hunger={hunger1:F1}), Bottom at {pos2} (hunger={hunger2:F1})");
            }
        }
        ...
        Assert.True(finalHunger1 > 20 || finalHunger2 > 20,
    ...
    public void TwoPawns_CrossingPaths_ShouldNotGetStuck()
    {
        ...
        for (int tick = 0; tick < 500; tick++)
        {
            ...
            if (tick % 50 == 0)
            {
                _output.WriteLine($"Tick {tick}: HungryPawn at {hungryPos} (hunger={hunger:F1}), TiredPawn at {tiredPos} (energy={energy:F1})");
                
                // Log actions
                if (sim.Entities.Actions.TryGetValue(hungryPawn.Value, out var hungryAction))
                {
                    var pathInfo = hungryAction.CurrentPath != null ? $"path len={hungryAction.CurrentPath.Count}, idx={hungryAction.PathIndex}" : "no path";
                    _output.WriteLine($"  HungryPawn action: {hungryAction.CurrentAction?.DisplayName ?? hungryAction.CurrentAction?.Type.ToString() ?? "none"}, {pathInfo}");
                }
                if (sim.Entities.Actions.TryGetValue(tiredPawn.Value, out var tiredAction))
                {
                    var pathInfo = tiredAction.CurrentPath != null ? $"path len={tiredAction.CurrentPath.Count}, idx={tiredAction.PathIndex}" : "no path";
                    _output.WriteLine($"  TiredPawn action: {tiredAction.CurrentAction?.DisplayName ?? tiredAction.CurrentAction?.Type.ToString() ?? "none"}, {pathInfo}");
                }
            }
        }
        ...
        _output.WriteLine($"HungryPawn got fed: {hungryGotFed}, TiredPawn got rested: {tiredGotRested}");
        ...
        Assert.True(uniqueHungryPositions.Count > 1 || uniqueTiredPositions.Count > 1,
    ...
    public void TwoPawns_DirectlyBlocking_EventuallyResolve()
    {
        ...
        for (int tick = 0; tick < 300; tick++)
        {
            ...
            if (tick % 50 == 0)
            {
                ...
                _output.WriteLine($"Tick {tick}: BlockedPawn at {pos1} (hunger={hunger:F1}), BlockerPawn at {pos2}");
            }
        }
    ...
    public void TwoPawns_WantSameObject_OneWaitsOrFindsAlternative()
    {
        ...
        for (int tick = 0; tick < 500; tick++)
        {
            ...
            {
                _output.WriteLine($"Tick {tick}: Sam at {samPos} (hygiene={samHygiene:F1}), Jordan at {jordanPos} (hygiene={jordanHygiene:F1})");
                ...
                if (sim.Entities.Actions.TryGetValue(sam.Value, out var samAction))
                {
                    ...
                    _output.WriteLine($"  Sam action: {samAction.CurrentAction?.DisplayName ?? samAction.CurrentAction?.Type.ToString() ?? "none"}, elapsed={elapsed}, duration={duration}");
                }
                if (sim.Entities.Actions.TryGetValue(jordan.Value, out var jordanAction))
                {
                    ...
                    _output.WriteLine($"  Jordan action: {jordanAction.CurrentAction?.DisplayName ?? jordanAction.CurrentAction?.Type.ToString() ?? "none"}, elapsed={elapsed}, duration={duration}");
                }
            }
        }
        ...
        var lastJordanPositions = jordanPositions.Skip(Math.Max(0, jordanPositions.Count - 100)).ToList();
        ...
        Assert.True(samShowered || jordanShowered, 
            "Neither pawn was able to use the shower");
        
        // At least one pawn should not be stuck in place forever
        Assert.True(uniqueSam.Count > 1 || uniqueJordan.Count > 1,
