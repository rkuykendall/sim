...
namespace SimGame.Core
{
    ...
    public static class PathAutotiler
    {
        ...
        {
            bool topLeft = IsAutotilingTile(
                world,
                new TileCoord(coord.X - 1, coord.Y - 1),
                pathTerrainId
            );
            bool topRight = IsAutotilingTile(
                world,
                new TileCoord(coord.X, coord.Y - 1),
                pathTerrainId
            );
            bool bottomLeft = IsAutotilingTile(
                world,
                new TileCoord(coord.X - 1, coord.Y),
                pathTerrainId
            );
            bool bottomRight = IsAutotilingTile(
                world,
        ...
        private static bool IsAutotilingTile(World world, TileCoord coord, int pathTerrainId)
        {
            ...
            return tile.OverlayTerrainTypeId.HasValue
                && tile.OverlayTerrainTypeId.Value == pathTerrainId;
        }
    }
}
